<script>
  import "../app.css";
  import "../app.scss";

  $: config = false

</script>



<div class="layout-main">
  <div class="lmn">
    <header></header>
    <main>
      <slot />
    </main>

    <nav>
      <div class="navigation">
        <ul class="flex-auto flex items-center">
          <div class="navigation">          
            <li>
              <button on:click={()=>{config =  !config}}>
                <img src="/logo.png" alt="logo "/>            
                </button>
                
            </li>
        
          </div>
        </ul>
      </div>
      <!-- start -->
      <article class:openconfig={config} class="config">
        <div class="art-config">
          <article class="config-page">
            <div class="cfg-page">
                
                <main>
                    <div class="mn">
                       
                        <a href="/home" on:click={()=>{config =  false}} class="home config-list">
                            <header>
                                <div class="hd">
                                    <span>
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M9.98996 22.75C9.78996 22.75 9.62996 22.71 9.50996 22.66C9.10996 22.51 8.42996 22.02 8.42996 20.47V14.02H6.08996C4.74996 14.02 4.26996 13.39 4.09996 13.02C3.92996 12.64 3.77996 11.87 4.65996 10.86L12.23 2.26001C13.25 1.10001 14.08 1.18001 14.48 1.33001C14.88 1.48001 15.56 1.97001 15.56 3.52001V9.97001H17.9C19.24 9.97001 19.72 10.6 19.89 10.97C20.06 11.35 20.21 12.12 19.33 13.13L11.76 21.73C11.05 22.54 10.43 22.75 9.98996 22.75ZM13.93 2.74001C13.9 2.78001 13.69 2.88001 13.36 3.26001L5.78996 11.86C5.50996 12.18 5.46996 12.38 5.46996 12.42C5.48996 12.43 5.66996 12.53 6.08996 12.53H9.17996C9.58996 12.53 9.92996 12.87 9.92996 13.28V20.48C9.92996 20.98 10.02 21.2 10.06 21.26C10.09 21.22 10.3 21.12 10.63 20.74L18.2 12.14C18.48 11.82 18.52 11.62 18.52 11.58C18.5 11.57 18.32 11.47 17.9 11.47H14.81C14.4 11.47 14.06 11.13 14.06 10.72V3.52001C14.07 3.02001 13.97 2.81001 13.93 2.74001Z" fill="black" fill-opacity="0.87"/>
                                        </svg>     
                                    </span><span>
                                        <h1>HOME</h1>
                                       
                                    </span>
                                </div>
                                <hr>
                            </header>
                            <main>
                                <div class="mn">
                                    <p>&hArr;</p>
                                </div>
        
                            </main>
                        </a>
                        <a href="/ads" on:click={()=>{config =  false}} class="ads config-list">
                            <header>
                                <div class="hd">
                                    <span>
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M9.98996 22.75C9.78996 22.75 9.62996 22.71 9.50996 22.66C9.10996 22.51 8.42996 22.02 8.42996 20.47V14.02H6.08996C4.74996 14.02 4.26996 13.39 4.09996 13.02C3.92996 12.64 3.77996 11.87 4.65996 10.86L12.23 2.26001C13.25 1.10001 14.08 1.18001 14.48 1.33001C14.88 1.48001 15.56 1.97001 15.56 3.52001V9.97001H17.9C19.24 9.97001 19.72 10.6 19.89 10.97C20.06 11.35 20.21 12.12 19.33 13.13L11.76 21.73C11.05 22.54 10.43 22.75 9.98996 22.75ZM13.93 2.74001C13.9 2.78001 13.69 2.88001 13.36 3.26001L5.78996 11.86C5.50996 12.18 5.46996 12.38 5.46996 12.42C5.48996 12.43 5.66996 12.53 6.08996 12.53H9.17996C9.58996 12.53 9.92996 12.87 9.92996 13.28V20.48C9.92996 20.98 10.02 21.2 10.06 21.26C10.09 21.22 10.3 21.12 10.63 20.74L18.2 12.14C18.48 11.82 18.52 11.62 18.52 11.58C18.5 11.57 18.32 11.47 17.9 11.47H14.81C14.4 11.47 14.06 11.13 14.06 10.72V3.52001C14.07 3.02001 13.97 2.81001 13.93 2.74001Z" fill="black" fill-opacity="0.87"/>
                                            </svg>
                                            
                                            
                                            
                                    </span><span>
                                        <h1>ADVERTISMENT</h1>
                                       
                                    </span>
                                </div>
                                <hr>
                            </header>
                            <main>
                                <div class="mn">
                                    <p>&hArr;</p>
                                </div>
        
                            </main>
                        </a>
                        <a href="/" on:click={()=>{config =  false}} class="download config-list">
                            <header>
                                <div class="hd">
                                    <span>
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M9.98996 22.75C9.78996 22.75 9.62996 22.71 9.50996 22.66C9.10996 22.51 8.42996 22.02 8.42996 20.47V14.02H6.08996C4.74996 14.02 4.26996 13.39 4.09996 13.02C3.92996 12.64 3.77996 11.87 4.65996 10.86L12.23 2.26001C13.25 1.10001 14.08 1.18001 14.48 1.33001C14.88 1.48001 15.56 1.97001 15.56 3.52001V9.97001H17.9C19.24 9.97001 19.72 10.6 19.89 10.97C20.06 11.35 20.21 12.12 19.33 13.13L11.76 21.73C11.05 22.54 10.43 22.75 9.98996 22.75ZM13.93 2.74001C13.9 2.78001 13.69 2.88001 13.36 3.26001L5.78996 11.86C5.50996 12.18 5.46996 12.38 5.46996 12.42C5.48996 12.43 5.66996 12.53 6.08996 12.53H9.17996C9.58996 12.53 9.92996 12.87 9.92996 13.28V20.48C9.92996 20.98 10.02 21.2 10.06 21.26C10.09 21.22 10.3 21.12 10.63 20.74L18.2 12.14C18.48 11.82 18.52 11.62 18.52 11.58C18.5 11.57 18.32 11.47 17.9 11.47H14.81C14.4 11.47 14.06 11.13 14.06 10.72V3.52001C14.07 3.02001 13.97 2.81001 13.93 2.74001Z" fill="black" fill-opacity="0.87"/>
                                            </svg>
                                            
                                            
                                            
                                    </span><span>
                                        <h1>DOWNLOAD</h1>
                                       
                                    </span>
                                </div>
                                <hr>
                            </header>
                            <main>
                                <div class="mn">
                                    <p>&hArr;</p>
                                </div>
        
                            </main>
                        </a>
                        <a href="/updates" on:click={()=>{config =  false}} class="faq config-list">
                            <header>
                                <div class="hd">
                                    <span>
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M9.98996 22.75C9.78996 22.75 9.62996 22.71 9.50996 22.66C9.10996 22.51 8.42996 22.02 8.42996 20.47V14.02H6.08996C4.74996 14.02 4.26996 13.39 4.09996 13.02C3.92996 12.64 3.77996 11.87 4.65996 10.86L12.23 2.26001C13.25 1.10001 14.08 1.18001 14.48 1.33001C14.88 1.48001 15.56 1.97001 15.56 3.52001V9.97001H17.9C19.24 9.97001 19.72 10.6 19.89 10.97C20.06 11.35 20.21 12.12 19.33 13.13L11.76 21.73C11.05 22.54 10.43 22.75 9.98996 22.75ZM13.93 2.74001C13.9 2.78001 13.69 2.88001 13.36 3.26001L5.78996 11.86C5.50996 12.18 5.46996 12.38 5.46996 12.42C5.48996 12.43 5.66996 12.53 6.08996 12.53H9.17996C9.58996 12.53 9.92996 12.87 9.92996 13.28V20.48C9.92996 20.98 10.02 21.2 10.06 21.26C10.09 21.22 10.3 21.12 10.63 20.74L18.2 12.14C18.48 11.82 18.52 11.62 18.52 11.58C18.5 11.57 18.32 11.47 17.9 11.47H14.81C14.4 11.47 14.06 11.13 14.06 10.72V3.52001C14.07 3.02001 13.97 2.81001 13.93 2.74001Z" fill="black" fill-opacity="0.87"/>
                                            </svg>
                                            
                                            
                                            
                                    </span><span>
                                        <h1>UPDATES</h1>
                                       
                                    </span>
                                </div>
                                <hr>
                            </header>
                            <main>
                                <div class="mn">
                                    <p>&hArr;</p>
                                </div>
        
                            </main>
                        </a>
        
                        <a href="tel://0880164455" class="supportus config-list">
                            <header>
                                <div class="hd">
                                    <span>
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M16.28 22.1H12.48C11.92 22.1 10.7 21.93 10.05 21.28L7.01999 18.94L7.93999 17.75L11.04 20.15C11.29 20.39 11.92 20.59 12.48 20.59H16.28C17.18 20.59 18.15 19.87 18.35 19.06L20.77 11.71C20.93 11.27 20.9 10.87 20.69 10.58C20.47 10.27 20.07 10.09 19.58 10.09H15.58C15.06 10.09 14.58 9.86999 14.25 9.48999C13.91 9.09999 13.76 8.57999 13.84 8.03999L14.34 4.82999C14.46 4.26999 14.08 3.63999 13.54 3.45999C13.05 3.27999 12.42 3.53999 12.2 3.85999L8.09999 9.95999L6.85999 9.12999L10.96 3.02999C11.59 2.08999 12.97 1.63999 14.05 2.04999C15.3 2.45999 16.1 3.83999 15.82 5.11999L15.33 8.26999C15.32 8.33999 15.32 8.43999 15.39 8.51999C15.44 8.56999 15.51 8.59999 15.59 8.59999H19.59C20.57 8.59999 21.42 9.00999 21.92 9.71999C22.41 10.41 22.51 11.32 22.19 12.2L19.8 19.48C19.43 20.93 17.89 22.1 16.28 22.1Z" fill="black" fill-opacity="0.87"/>
                                            <path d="M5.38 21H4.38C2.53 21 1.63 20.13 1.63 18.35V8.55002C1.63 6.77002 2.53 5.90002 4.38 5.90002H5.38C7.23 5.90002 8.13 6.77002 8.13 8.55002V18.35C8.13 20.13 7.23 21 5.38 21ZM4.38 7.40002C3.29 7.40002 3.13001 7.66002 3.13001 8.55002V18.35C3.13001 19.24 3.29 19.5 4.38 19.5H5.38C6.47001 19.5 6.63 19.24 6.63 18.35V8.55002C6.63 7.66002 6.47001 7.40002 5.38 7.40002H4.38Z" fill="black" fill-opacity="0.87"/>
                                            </svg>
                                            
                                            
                                            
                                    </span><span>
                                        <h1>contact us</h1>
                                    </span>
                                </div>
                                <hr>
                            </header>
                            <main>
                                <div class="mn">
                                    <p>&hArr;</p>
                                </div>
        
                            </main>
                        </a>
                    </div>
                </main>
        
            </div>
        </article>
        </div>
      </article>



    </nav>





    <div class="bottom">
      <div class="ft">
        <div class="upperfoot">
        <div class="up-ft">
            <div class="svg">
            <svg width="46" height="6" viewBox="0 0 46 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect width="6" height="6" rx="3" fill="#2BB731"/>
                <rect x="20" width="6" height="6" rx="3" fill="#2BB731"/>
                <rect x="40" width="6" height="6" rx="3" fill="#2BB731"/>
            </svg>
            </div>
            <div>
            <button>
                <a href="tel:+265880164455">REQUEST PERSONAL FEATURES</a>
            </button>
            </div>
            <div class="svg">
            <svg width="50" height="1" viewBox="0 0 50 1" fill="none" xmlns="http://www.w3.org/2000/svg">
            <rect width="50" height="1" rx="0.5" fill="#2BB731"/>
            </svg>
            </div>
        </div>
        </div>
        <div class="lowwerfoot">
        <div>
            <div class="github">
            <a href="https://github.com/developerafrica">
                <p>
                <span>found an issue on this page ?</span><span>fix it on github</span>
                </p>
                <i><svg width="50" height="50" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M16.7238 38.8086C16.7238 39.0039 16.4919 39.1602 16.1996 39.1602C15.8669 39.1895 15.6351 39.0332 15.6351 38.8086C15.6351 38.6133 15.8669 38.457 16.1593 38.457C16.4617 38.4277 16.7238 38.584 16.7238 38.8086ZM13.5887 38.3691C13.5181 38.5645 13.7198 38.7891 14.0222 38.8477C14.2843 38.9453 14.5867 38.8477 14.6472 38.6523C14.7077 38.457 14.5161 38.2324 14.2137 38.1445C13.9516 38.0762 13.6593 38.1738 13.5887 38.3691ZM18.0444 38.2031C17.752 38.2715 17.5504 38.457 17.5806 38.6816C17.6109 38.877 17.873 39.0039 18.1754 38.9355C18.4677 38.8672 18.6694 38.6816 18.6391 38.4863C18.6089 38.3008 18.3367 38.1738 18.0444 38.2031ZM24.6774 0.78125C10.6956 0.78125 0 11.0645 0 24.6094C0 35.4395 7.03629 44.707 17.0867 47.9688C18.377 48.1934 18.8306 47.4219 18.8306 46.7871C18.8306 46.1816 18.8004 42.8418 18.8004 40.791C18.8004 40.791 11.744 42.2559 10.2621 37.8809C10.2621 37.8809 9.1129 35.0391 7.45968 34.3066C7.45968 34.3066 5.15121 32.7734 7.62097 32.8027C7.62097 32.8027 10.131 32.998 11.5121 35.3223C13.7198 39.0918 17.4194 38.0078 18.8609 37.3633C19.0927 35.8008 19.748 34.7168 20.4738 34.0723C14.8387 33.4668 9.15323 32.6758 9.15323 23.2812C9.15323 20.5957 9.91936 19.248 11.5323 17.5293C11.2702 16.8945 10.4133 14.2773 11.7944 10.8984C13.9012 10.2637 18.75 13.5352 18.75 13.5352C20.7661 12.9883 22.9335 12.7051 25.0806 12.7051C27.2278 12.7051 29.3952 12.9883 31.4113 13.5352C31.4113 13.5352 36.2601 10.2539 38.3669 10.8984C39.748 14.2871 38.8911 16.8945 38.629 17.5293C40.2419 19.2578 41.2298 20.6055 41.2298 23.2812C41.2298 32.7051 35.2923 33.457 29.6573 34.0723C30.5847 34.8437 31.371 36.3086 31.371 38.6035C31.371 41.8945 31.3407 45.9668 31.3407 46.7676C31.3407 47.4023 31.8044 48.1738 33.0847 47.9492C43.1653 44.707 50 35.4395 50 24.6094C50 11.0645 38.6593 0.78125 24.6774 0.78125ZM9.79839 34.4629C9.66734 34.5605 9.69758 34.7852 9.86895 34.9707C10.0302 35.127 10.2621 35.1953 10.3931 35.0684C10.5242 34.9707 10.494 34.7461 10.3226 34.5605C10.1613 34.4043 9.92944 34.3359 9.79839 34.4629ZM8.70968 33.6719C8.63911 33.7988 8.73992 33.9551 8.94153 34.0527C9.10282 34.1504 9.30444 34.1211 9.375 33.9844C9.44556 33.8574 9.34476 33.7012 9.14314 33.6035C8.94153 33.5449 8.78024 33.5742 8.70968 33.6719ZM11.9758 37.1484C11.8145 37.2754 11.875 37.5684 12.1069 37.7539C12.3387 37.9785 12.631 38.0078 12.7621 37.8516C12.8931 37.7246 12.8327 37.4316 12.631 37.2461C12.4093 37.0215 12.1069 36.9922 11.9758 37.1484ZM10.8266 35.7129C10.6653 35.8105 10.6653 36.0645 10.8266 36.2891C10.9879 36.5137 11.2601 36.6113 11.3911 36.5137C11.5524 36.3867 11.5524 36.1328 11.3911 35.9082C11.25 35.6836 10.9879 35.5859 10.8266 35.7129Z" fill="black"/>
                </svg>
                </i>
            </a>
            </div>
            <article >
            <div class="art-au">
                <span>created with 🖤 by</span><span>_</span><span><a href="https://github.com/Peterbutao">BUTAO PETER | UX / UI DESIGNER</a></span>
            </div>
            </article>
            <div class="copy">
            <a href="https://developerafrica.netlify.app">copyright ©  developerafricamw | 2022</a>
            </div>
        </div>
        </div>
    </div>
    <!-- end -->
        
    </div>



    
  </div>
</div>
<style lang="scss">
    .openconfig{
        pointer-events: all !important;
        opacity: 100% !important;
        clip-path: circle(1500px) !important;
        transition: all 1s ease-in-out !important;
    }
  .config{
    background: white;
    position: fixed;
    height: 100vh;
    width: 100%;
    top: 0;
    left: 0;
    z-index: 20;
    pointer-events: none;
    opacity: 0;
    clip-path: circle(0);
    .cfg-page{
            padding: 2rem 1rem 1rem 1rem;
            
            main{
                padding: 20px 10px;
                .mn{
                    .config-list{
                        margin: 2rem 0;
                        display: block;
                        header{
                            .hd{
                                display: flex;
                                align-items: center;
                                padding: 1rem 0;
                                h1{
                                    @include font(var(--tc), 1.5rem, 600);
                                    padding: 0 0.5rem;
                                }
                            }
                        }
                    }
                    .app-settings{
                        main{
                            .mn{
                                .theme-toggle{
                                    padding: 1.1rem 0.5rem;
                                    button{
                                        background: var(--l-gr);
                                        border-radius: 4px;
                                        display: flex;
                                        align-items: center;
                                        padding: 0.5rem;
                                        box-shadow: rgba(0, 0, 0, 0.22) 0 4px 4px 0;
                                        p{
                                            @include font(var(--tc), 1.3rem, 600);
                                            padding: 0 0.5rem;
                                        }
                                        svg{
                                            width: 18px;
                                        }
                                    }
                                }
                            }
                        }

                    }
                    
                }
            }
        }
  }
  .layout-main{
    .lmn{
      nav{
        position: fixed;
        left:0;
        top:0;
        width: 100%;
        z-index: 30;
        .navigation{
            ul{
                padding: 1rem 0.5rem;
                width: 100%;
                
                .navigation{
                    width: 100%;
                    button{
                        padding: 0.5rem 1rem;
                        position: fixed;
                        top:0;
                        right:0;

                        width: 100%;
                        z-index: 40;

                img{
                  width: 40px;
                }
              }

            }
          }
        }
       
      }
    }
  }
  .ft{
    text-align: center;
    .upperfoot{
        padding: 1rem;
        .svg{
            padding: 1rem 0;
            display: grid;
            place-items: center;

        }
        button{
            border: var(--gn) solid 1px;
            border-radius: 4px;
            padding: 1.3rem;
            background: none;
            @include font(var(--tc), 1.3rem , 500);
        }
    }
    .lowwerfoot{
        padding: 1rem;
        text-align: center;
        .github{
          
            padding: 2rem;
            background: rgba(128, 128, 128, 0.422);
            border-radius: 1px;
            i{
              padding: 10px;
              display: grid;
              place-items: center;
            }
            a{
                @include font(var(--tc), 1.3rem, 500);
                span:nth-child(2){
                    color: var(--bl);
                    margin: 0 2px;
                }
            }
        }
        .copy{
            padding: 0 0 10rem 0;
            a{
                @include font(var(--tc), 1.4rem, 500);
            }
        }
        article{
            @include font(var(--tc), 1.2rem, 500);
            padding: 4rem 0 1rem 0;
            span:nth-child(3){
                color: var(--bl);
                margin: 0 2px;
            }
        }
    }
}
</style>